...
export class AppComponent {
  ...
  mySource = (keyword: any) => {
    var data = {
      keyword: this.codeArray.join("\n") + "\n" + keyword,
      model: this.activeModel
    };
    return this.localService.predict(data).map(data => {
      return data.json();
    });
  };
  ...
  getModels() {
    return this.localService.getModels().subscribe(
      data => (this.lstmModels = data.json()["data"]["results"]),
